...
OUTPUT_DIR=$(date +mpileup_%Y_%m_%d_%H_%M)
...
find ./ -name "*.bam" > $OUTPUT_DIR/mpileup_reference
...
#SBATCH -c 1
#SBATCH --mem 32000
#SBATCH -o ./$OUTPUT_DIR/run_logs/mpileup.%N.%j.out
#SBATCH -e ./$OUTPUT_DIR/run_logs/mpileup.%N.%j.err
...
" > ./$OUTPUT_DIR/slurm_scripts/mpileup.sh
JOBID=$(sbatch ./$OUTPUT_DIR/slurm_scripts/mpileup.sh | \
    awk '{print($4)}')
